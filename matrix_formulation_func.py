...
class Matrix_Formulation_Class(QObject):
  finished = pyqtSignal()
  return_message = pyqtSignal(str)
  nx_total_message = pyqtSignal(int)
  ...
  def matrix_formulation(self):
    ...
    if(self.sp_step == None and self.given_disc_point==None):
      self.return_message.emit("Error no discretization method is choosen!")
      self.finished.emit()
    ...
    try:
      with open(csv_filename) as f:
        ...
        for line in lines:
          ...
          if line[0] == 'R':
            ...
            if length < dx:
              self.return_message.emit("The given discretization is not dense enough for the geometry of the given line.")
              self.finished.emit()
      ...
      self.return_message.emit("Corrupted CSV segments file.")
      self.finished.emit()
      print("Corrupted CSV segments file.")
      return
    except FileNotFoundError as e:
      self.return_message.emit("CSV file not found.")
      self.finished.emit()
      ...
      print(e)
      traceback.print_exc()
      self.return_message.emit("An error occured while parsing CSV file.")
      self.finished.emit()
      print("An error occured while parsing CSV file.")
      return

    self.nx_total_message.emit(nx_total)
    ...
    try:
      ...
      self.finished.emit()
      self.return_message.emit(f"File {filename} could not be opened. Check if it is opened by another application.")
      print(f"File {filename} could not be opened. Check if it is opened by another application.")
      return
    except FileNotFoundError as e:
      self.finished.emit()
      self.return_message.emit(f"File {filename} was not found in the system. Try performing discretization again.")
      print(f"File {filename} was not found in the system. Try performing discretization again.")
      return
    except Exception as e:
      self.finished.emit()
      self.return_message.emit(f"An error occured while writing file {filename}.")
